...
const kafkaegress = async (topic, message) => {
    ...
    try {
        ...
        const temp = await db('devices_group').select('devices_group.name as group', 'devices_group.coordinates as coordinates', 'devices_group.id as group_id', 'devices_group.longitude', 'devices_group.latitude').where('id', '=', db('groups').select('group_id').where({
            "device_id": message.device_id,
            "sensor_type": message.sensor_type,
        }))

...
module.exports.deviceQuery = {
    devicebyId: async (parent, args, info, context) => {
       ...
       const [device]=await  db.select('devices.id', 'devices.sensor_type', 'groups.device_id', 'devices_group.name', 'devices_group.coordinates', 'devices_group.latitude', 'devices_group.longitude')
  .from('groups')
  ...
  .where({
    ...
    'groups.sensor_type': args.sensor_type
        ...
        return {id:device.id,sensor_type:device.sensor_type,group:{name:device.name,coordinates:device.coordinates,long:device.longitude,lat:device.latitude}}
    },
